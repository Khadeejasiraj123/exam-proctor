...
module.exports = (db) => {
  ...
  router.get("/admin/:id", function (req, res) {
    console.log("proctor appointments dashboard backend", req.params.id);
    console.log("proctor appointments dashboard backend");
    const id = req.params.id;
    console.log("proctor appointments dashboard backend", id);

    console.log(
      `SELECT appointments.*, tests.type 
      FROM appointments 
      JOIN tests ON appointments.test_id=tests.id 
      WHERE proctor_id = ${req.params.id} and start_date = CURRENT_DATE ;`
    );
